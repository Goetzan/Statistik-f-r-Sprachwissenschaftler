...
for(s in c(3,4,5) ){
  ...
  output <- paste("Bei einer Standabweichung von",s, "bestehen",sehr.gut*100,"% sehr gut.")
  print(output)
}
```
...
Es sind mehr (doppelt soviele) schlechte Noten (0-5 Notenpunkte), als gute Noten(nur 13-15 Notenpunkte) in die Berechnung mit eingeflossen. Aus diesesm Grund ist die Durchfallquote höher.

Warum?

Da es möglich ist insgesamt mehr schlechte Noten zu erhalten, als Gute ist das Verhältnis und die Berechnung nicht ausgewogen. Um ein einheitlicheres und dmait vergleichbares Ergebnis zu erzielen müssten beispielsweise nur die 1er (13-15 NP) mit den 5ern (1-3) verglichen werden.
...
2. Wie viele Studenten bekommen zumindest 10 NP?
    ...
    `r (pnorm(9,mean=mu,sd=3, lower.tail=FALSE) * n)` Studenten bekommen zumindest 10 NP.

3. Wie viele Studenten bekommen weniger als 10 NP?

    `r (pnorm(9,mean=mu,sd=3) * n)` Studenten bekommen weniger als 10 NP.
...
Der kritische Wert für einen einseitigen $z$-Test ist (bei Alpha=0,06) `r qnorm(0.94)` (bei Alpha=0,05) `r qnorm(0.95)`.

Die kritischen Werte für einen zweiseitigen $z$-Test sind (bei Aplha=0,06) $\pm$ `r qnorm(0.97)` (bei Alpha=0,05) $\pm$ `r qnorm(0.975)`.
...
Bei Unsicherheiten ist es besser einen zweiseitigen z-Test auszuführen, da sonst die der Fehler gemacht werden kann, dass die Hypothese abgelehnt wird obwohl sie richtig ist.
...
Das ist ein **_insignifikanter_** Unterschied, da 1.764<1.8808. 
...
``` {r}
z.n50 <- sqrt(50) * ((10 - 8)/3)
print(z.n50)

```

Das ist ein **_signifikanter_** Unterschied, da 4.714>1.8808. 

Ist die Benotung vom Dozenten weniger als ideal? 

### Ein anderer, böserer? Dozent
Die Verwaltung ist auch auf einen anderen Dozenten aufmerksam geworden, weil manche Studenten behaupten, er würde zu streng benoten. (Welche Testart sollte man hier nutzen?) Einseitiger z-Test
...
Das ist ein **_insignifikanter_** Unterschied, da 1.49<1.55.
...
Das ist ein **_signifikanter_** Unterschied, da 1.66>1.55.
