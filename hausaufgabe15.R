...
body.metric <- body
body.metric$height.cm <- body.metric$height
body.metric$weight.kg <- body.metric$weight
ggplot(body.metric,aes(x=height.cm,y=weight.kg)) +  geom_point() + geom_smooth(method="lm")
m4 <- lm(weight.kg ~ height.cm, data=body.metric)
print(summary(m4))
